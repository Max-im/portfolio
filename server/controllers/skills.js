...
export const getSkills = (req, res) => {
  db.query(
    `SELECT s.name, s.id, s.icon, s.url, c.name AS category 
    FROM skills AS s 
...
//   client
//     .query(`SELECT skill, skill_picture, source, range, category_id FROM skills WHERE id=$1`, [
//       req.body.id
//     ])
//     .then(({ rows }) => {
...
//       return client.query(`UPDATE skills SET ${key}=$1 WHERE id=$2`, [toUpdate[key], id]);
//     })
//   )
//     .then(() => res.end())
//     .catch(err => res.status(400).json(err));
...
//   client
//     .query(`DELETE FROM skills WHERE category_id=$1 RETURNING id`, [req.params.id])
//     .then(() => next())
//     .catch(err => res.status(400).json(err));
// };
...
//   client
//     .query(`DELETE FROM skills WHERE id=$1 RETURNING skill_picture`, [req.params.id])
//     .then(({ rows }) => {
